    ...
    public class SyntheticController : ControllerBase
    {
        ...
        public async Task<IActionResult> GenerateJsonl([FromBody] GenerateJsonlRequest request)
        {
            try
            {
                ...
                var userPrompt = $@"以下はPDFのページごとの差分説明です。これを参考に、未知の文書に対して構造化テキストを生成するfew-shot用の合成データを{n}行のJSONLで作成してください。
...
差分まとめ:\n\n{diffsJoined}";
                ...
                var content = await _anthropic.GenerateChatResponseAsync(userPrompt, context: string.Empty, systemPrompt: system);
